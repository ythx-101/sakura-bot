...
class BotConfig:
    def __init__(self, path_config_file: str):
        ...
        if self.use_proxy == "1":
            ...
            # Parse proxy address for PikPak (handle socks5:// format)
            if self.proxy_addr.startswith("socks5://"):
                # For SOCKS5, use the proxy as-is for requests
                # PikPak might not work with SOCKS5, so disable it
                self.proxy_json_pikpak = {}
            else:
                # For HTTP proxy format
...
# Monkey-patch: fix DMM grid index change
from dmm_patch import patched_get_nice_avs_by_star_name
import types as _types_builtin
DMM_UTIL.get_nice_avs_by_star_name = _types_builtin.MethodType(patched_get_nice_avs_by_star_name, DMM_UTIL)

# DMM Cookie for authentication
DMM_COOKIE = os.environ.get("DMM_COOKIE", "")

# Configure DMM_UTIL to use cookie authentication
try:
    if hasattr(DMM_UTIL, "session"):
        DMM_UTIL.session.headers["Cookie"] = DMM_COOKIE
        print("DMM Cookie authentication configured successfully")
    else:
        print("Warning: DMM_UTIL session not available for cookie setting")
except Exception as e:
    print(f"Warning: Could not set DMM cookie: {e}")
...
BOT_CMDS = {
    ...
    "embed": "Index a video file with Gemini Embedding 2 (path)",
    "clips": "Search video clips by description",
...
class BotUtils:
    ...
    def create_btn_by_key(self, key_type: str, obj):
        ...
        elif key_type == "VIDEO_RANKING":
            # Extract video ID from the string format "SSIS-001 ★★★★★"
            video_id = obj.split(" ")[0]  # Get just the video ID part
            return InlineKeyboardButton(text=obj, callback_data=f"{video_id}:k2_0")
    ...
    def get_v_by_id(
        ...
        v_magnets = v["magnets"]
        # 按文件大小降序排序
        v_magnets = sorted(v_magnets, key=lambda x: x.get("size_bytes", 0), reverse=True)
        v_url = v["url"]
        msg = ""
        if v_title != "":
...
"""
        ...
        # Add PikPak save button
        pikpak_btn = InlineKeyboardButton(
            text="📁 Save to PikPak",
            callback_data=f"{v_id}:{BotKey.KEY_SAVE_TO_PIKPAK}"
        )
        if len(v_magnets) != 0:
            markup = InlineKeyboardMarkup()
            markup.row(sample_btn, pv_btn, more_btn)
            markup.row(pikpak_btn)
        else:
            markup = InlineKeyboardMarkup()
            markup.row(sample_btn, pv_btn)
            markup.row(pikpak_btn)
    ...
    def get_top_stars(self, page=1):
        op_get_top_stars = f"Get Popular Actress Ranking"
        cache_key = f"actress_ranking_{page}"
        stars = BOT_CACHE_DB.get_cache(key=cache_key, type=BotCacheDb.TYPE_RANK)

        if not stars:
            # 实时从 JavBus 获取人气女优排行
            from fallback_sources import get_top_stars as _get_top
            code, live_stars = _get_top(BOT_CFG.proxy_addr, limit=20)
            if code == 200 and live_stars:
                stars = live_stars[:20]
            else:
                # fallback 写死名单
                stars = ["三上悠亜", "深田えいみ", "橋本ありな", "桃乃木かな",
                         "天使もえ", "葵つかさ", "明里つむぎ", "相沢みなみ"]
            BOT_CACHE_DB.set_cache(key=cache_key, value=stars, type=BotCacheDb.TYPE_RANK, expire=21600)
            LOG.info(f"Generate popular actress ranking: {len(stars)} actresses")

        return stars
    def show_actress_ranking(self, page=1):
        """Display DMM Popular Actress Ranking with UI"""
        stars = self.get_top_stars(page)
        
        # Create ranking with emojis
        ranking_icons = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣", "6️⃣", "7️⃣", "8️⃣"]
        
        # Prepare ranking display with formatted names
        ranking_display = []
        for i, star_name in enumerate(stars):
            icon = ranking_icons[i] if i < len(ranking_icons) else f"{i+1}️⃣"
            ranking_display.append(f"{icon} {star_name}")
        
        # Create title with current time
        import datetime
        current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        title = f"<b>🎌 DMM热门女优排行榜</b>\n\n" + "\n".join(ranking_display) + f"\n\n<i>更新时间: {current_time}</i>"
        
        # Use send_msg_btns to create clickable actress buttons
        self.send_msg_btns(
            max_btn_per_row=2,
            max_row_per_msg=10, 
        ...
        LOG.info(f"Displayed actress ranking with {len(stars)} actresses")
    ...
    def send_msg_to_pikpak(self, msg):
        ...
        try:
            ...
            for u in url_candidates:
                try:
                    import requests
                    r = requests.post(u, json={"msg": msg}, timeout=4)
    ...
    def get_more_magnets_by_id(self, id: str):
        ...
        # 按文件大小降序排序
        magnets = sorted(magnets, key=lambda x: x.get("size_bytes", 0), reverse=True)
        msg = ""
...
"""
    ...
    def random_get_new_v(self):
        from fallback_sources import get_random_new_id
        code, id = get_random_new_id(BOT_CFG.proxy_addr)
        if self.check_success(code, "Randomly Fetch Latest"):
            self.get_v_by_id(id=id)

    def random_get_nice_v(self):
        from fallback_sources import get_random_nice_id
        code, id = get_random_nice_id(BOT_CFG.proxy_addr)
    ...
    def random_get_nice_star_vs(self, star_name_ori):
        ...
        if not vs:
            ...
            code, vs = DMM_UTIL.get_nice_avs_by_star_name(star_name=star_name_ja)
...
def handle_callback(call):
    ...
    if key_type == BotKey.KEY_WATCH_PV_BY_ID:
        ...
        bot_utils.get_v_by_id(id=content)
    elif key_type == BotKey.KEY_SAVE_TO_PIKPAK:
        # Handle PikPak save request
        v_id = content
        LOG.info(f"Processing PikPak save request for video ID: {v_id}")
        
        # First try to get from cache
        v = BOT_CACHE_DB.get_cache(key=v_id, type=BotCacheDb.TYPE_V)
        if not v:
            LOG.info(f"Video not in cache, fetching fresh data for: {v_id}")
            # Get video info to retrieve magnet link
            # 获取所有磁力链接（不过滤），用于选择最大的
            v = bot_utils.get_v_by_id(id=v_id, not_send=True, is_nice=False, is_uncensored=False, magnet_max_count=999)
        
        if v and "magnets" in v and len(v["magnets"]) > 0:
            # 按文件大小降序排序，取最大的（最高清）
            sorted_magnets = sorted(v["magnets"], key=lambda x: x.get("size_bytes", 0), reverse=True)
            magnet_link = sorted_magnets[0]["link"]
            magnet_size = sorted_magnets[0].get("size", "Unknown")
            LOG.info(f"Found {len(v['magnets'])} magnets, selected largest: {magnet_size}")
            LOG.info(f"Magnet link: {magnet_link[:50]}...")
            # Send to PikPak
            result = bot_utils.send_msg_to_pikpak(magnet_link)
            if result is not None:
                bot_utils.send_msg(f"✅ Successfully saved <code>{v_id}</code> to PikPak")
                LOG.info(f"Successfully sent magnet to PikPak for video: {v_id}")
            else:
                bot_utils.send_msg(f"❌ Failed to save <code>{v_id}</code> to PikPak. Please check network connection and logs.")
                LOG.error(f"Failed to send magnet to PikPak for video: {v_id}")
        else:
            LOG.warning(f"No magnet links found for video ID: {v_id}")
            bot_utils.send_msg(f"❌ No magnet links available for <code>{v_id}</code>. Please try searching for the video again.")
...
def handle_message(message):
    try:
        ...
        if msg_cmd == "/help" or msg_cmd == "/start":
            ...
            bot_utils.show_actress_ranking(1)
        elif msg_cmd == "/star":
            if msg_param:
            ...
            if msg_param:
                threading.Thread(target=_handle_clips, args=(bot_utils, msg_param)).start()
            else:
                bot_utils.send_msg("用法: /clips <搜索描述>，如 /clips 室内特写")
        elif msg_cmd == "/weekly":
...
def _handle_clips(bot_utils, query: str):
    """文字搜封面索引，返回番号+标题+女优"""
    import sys
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    try:
        from jav_cover_search import search_by_text, format_results, load_index, get_text_embedding, load_config, get_api_key
        entries = load_index()
        if not entries:
            bot_utils.send_msg("\U0001f4ed 封面索引为空，watcher 处理新视频后自动建立")
            return
        bot_utils.send_msg(f"\U0001f50d 搜索封面索引（{len(entries)} 个番号）...")
        results = search_by_text(query, top_n=5)
        # 记录历史向量用于推荐
        try:
            cfg = load_config()
            emb = get_text_embedding(query, get_api_key(cfg))
            from jav_recommend import record_query
            record_query(query, emb)
        except Exception:
            pass
        bot_utils.send_msg(format_results(results, query))
    except Exception as e:
        LOG.error(f"_handle_clips error: {e}")
        bot_utils.send_msg(f"\u274c /clips 出错: {e}")
...
def _handle_recommend(bot_utils):
    ...
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    try:
        from jav_recommend import recommend, load_history, format_recommend_msg
        history = load_history()
        if not history:
            bot_utils.send_msg("📭 还没有搜索历史，先用 /clips 搜索一些片段")
            return
        bot_utils.send_msg(f"🔄 基于 {len(history)} 条历史记录计算推荐...")
        results = recommend(top_n=5)
        bot_utils.send_msg(format_recommend_msg(results, len(history)))
    except Exception as e:
        LOG.error(f"_handle_recommend error: {e}")
